...
function getVariograma(x, y, z) { 
    ...
    for (var i = 0; i < Y.length; i++) {
        //---X[i][1] = 1.0 - Math.exp(-(1.0 / (0.33)) * lagsemi[i] / variograma.rango); // 1.0 - Math.exp(-(1.0 / A) * lagsemi[i] /variograma.rango );
        X[i][1] = 1.0 - Math.exp(-(1.0 / (0.33)) * lagsemi[i] / rango);
    ...
    C1=parseFloat(W[1])+parseFloat(C0)
