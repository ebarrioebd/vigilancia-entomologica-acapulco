...
function estimar(lat, long, variograma) { 
    ...
    var pesos = mult(variograma.K,_Y)//math.multiply(variograma.K, math.transpose(_Y)); 
    return mult(transpose(pesos),variograma.z)
...
function getVariograma(x, y, z) { 
    ...
    variograma.w0 = W[0];
    variograma.w1 = W[1];
    //c("Nugget:", variograma.nugget)
    //c("sill:", variograma.sill)
    ...
    for (var i = 0; i < n; i++) {
        for (var j = 0; j < n; j++) {
            //K[i][j] = W[0][0] + W[1][0]* (1.0 - Math.exp(-(1.0 / A) * (  (Math.sqrt(Math.pow(x[j] - x[i], 2) + Math.pow(y[j] - y[i], 2))*10000)  / parseFloat(variograma.rango)) )  )
            K[i][j] =  W[0][0] + W[1][0] * (1.0 - Math.exp(-(1.0 / A) * ( Math.sqrt(Math.pow(x[j] - x[i], 2) + Math.pow(y[j] - y[i], 2))*100000/ variograma.rango)))
            ///turfDistanciaturf.distance([x[i],y[i]],[x[j],y[j]],{units: 'meters'})
            //K[i][j] = W[0] + W[1] * (1.0 - Math.exp(-(1.0 / A) * (turf.distance([x[i], y[i]], [x[j], y[j]], units) / variograma.rango)))
        }
        z[i]=[z[i]]
    } 
    variograma.K = invM(K) //math.inv(K); //matriz inversa de los valores del variograma teorico y^-1(xi-xj) xi=[lat,long] 
    //variograma.M = mult(variograma.K, z) //math.multiply(variograma.K, z) 
    return variograma; 
};
